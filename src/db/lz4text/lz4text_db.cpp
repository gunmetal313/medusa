...
LZ4TextDatabase::LZ4TextDatabase(void) : m_DirtyLabels(false), m_IsIteratingLabels(false)
{
}

LZ4TextDatabase::~LZ4TextDatabase(void)
{
...
std::string LZ4TextDatabase::GetName(void) const
{
  return "LZ4Text";
}

std::string LZ4TextDatabase::GetExtension(void) const
{
  return ".mlz";
}

bool LZ4TextDatabase::IsCompatible(boost::filesystem::path const& rDatabasePath) const
{
...
bool LZ4TextDatabase::Open(boost::filesystem::path const& rDatabasePath)
{
...
bool LZ4TextDatabase::Create(boost::filesystem::path const& rDatabasePath, bool Force)
{
  if (!m_DatabasePath.string().empty())
  {
...
bool LZ4TextDatabase::Flush(void)
{
  if (m_DatabasePath.string().empty())
    return false;
  ...
  TextFile << std::hex << std::showbase << "# Medusa LZ4 Text Database\n";

  // Save binary stream
  {
    TextFile << "## LZ4 BinaryStream\n";
	//compress binary stream and to base64
	char* pchCompressed = new char[m_spBinStrm->GetSize()];
	void * input = m_spBinStrm->GetBuffer();
	char *pc;
	pc = static_cast<char*>(input);
	int nCompressedSize = LZ4_compress_default(pc, pchCompressed, m_spBinStrm->GetSize(), LZ4_MAX_INPUT_SIZE);
	std::string Base64Data = Base64Encode(pchCompressed, sizeof(pchCompressed));
    TextFile << Base64Data << "\n" << std::flush;
  }
...
bool LZ4TextDatabase::Close(void)
{
  bool Res = Flush();
...
bool LZ4TextDatabase::RegisterArchitectureTag(Tag ArchitectureTag)
{
  std::lock_guard<std::mutex> Lock(m_ArchitectureTagLock);
...
bool LZ4TextDatabase::UnregisterArchitectureTag(Tag ArchitectureTag)
{
  std::lock_guard<std::mutex> Lock(m_ArchitectureTagLock);
...
std::list<Tag> LZ4TextDatabase::GetArchitectureTags(void) const
{
  std::lock_guard<std::mutex> Lock(m_ArchitectureTagLock);
  return m_ArchitectureTags;
}

bool LZ4TextDatabase::AddMemoryArea(MemoryArea* pMemArea)
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
  m_MemoryAreas.insert(pMemArea);
  return true;
...
void LZ4TextDatabase::ForEachMemoryArea(MemoryAreaCallback Callback) const
{
  for (auto const pMemArea : m_MemoryAreas)
    Callback(*pMemArea);
}

MemoryArea const* LZ4TextDatabase::GetMemoryArea(Address const& rAddress) const
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
  for (MemoryArea* pMemArea : m_MemoryAreas)
...
bool LZ4TextDatabase::GetFirstAddress(Address& rAddress) const
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
...
bool LZ4TextDatabase::GetLastAddress(Address& rAddress) const
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
  if (m_MemoryAreas.empty())
    return false;
...
bool LZ4TextDatabase::MoveAddress(Address const& rAddress, Address& rMovedAddress, s64 Offset) const
{
  if (Offset < 0)
...
bool LZ4TextDatabase::ConvertAddressToPosition(Address const& rAddress, u32& rPosition) const
{
  rPosition = 0;
...
bool LZ4TextDatabase::ConvertPositionToAddress(u32 Position, Address& rAddress) const
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
  for (MemoryArea* pMemArea : m_MemoryAreas)
  {
...
bool LZ4TextDatabase::AddLabel(Address const& rAddress, Label const& rLabel)
{
  std::lock_guard<std::recursive_mutex> Lock(m_LabelLock);
...
bool LZ4TextDatabase::RemoveLabel(Address const& rAddress)
{
  std::lock_guard<std::recursive_mutex> Lock(m_LabelLock);
...
bool LZ4TextDatabase::GetLabel(Address const& rAddress, Label& rLabel) const
{
  std::lock_guard<std::recursive_mutex> Lock(m_LabelLock);
  auto itLbl = m_LabelMap.left.find(rAddress);
  if (itLbl == std::end(m_LabelMap.left))
...
bool LZ4TextDatabase::GetLabelAddress(Label const& rLabel, Address& rAddress) const
{
  std::lock_guard<std::recursive_mutex> Lock(m_LabelLock);
...
void LZ4TextDatabase::ForEachLabel(LabelCallback Callback)
{
...
bool LZ4TextDatabase::AddCrossReference(Address const& rTo, Address const& rFrom)
{
  std::lock_guard<std::mutex> Lock(m_CrossReferencesLock);
  return m_CrossReferences.AddXRef(rTo, rFrom);
}

bool LZ4TextDatabase::RemoveCrossReference(Address const& rFrom)
{
  std::lock_guard<std::mutex> Lock(m_CrossReferencesLock);
  return m_CrossReferences.RemoveRef(rFrom);
}

bool LZ4TextDatabase::RemoveCrossReferences(void)
{
  std::lock_guard<std::mutex> Lock(m_CrossReferencesLock);
...
bool LZ4TextDatabase::HasCrossReferenceFrom(Address const& rTo) const
{
  std::lock_guard<std::mutex> Lock(m_CrossReferencesLock);
  return m_CrossReferences.HasXRefFrom(rTo);
}

bool LZ4TextDatabase::GetCrossReferenceFrom(Address const& rTo, Address::List& rFromList) const
{
  std::lock_guard<std::mutex> Lock(m_CrossReferencesLock);
  return m_CrossReferences.From(rTo, rFromList);
}

bool LZ4TextDatabase::HasCrossReferenceTo(Address const& rFrom) const
{
  std::lock_guard<std::mutex> Lock(m_CrossReferencesLock);
  return m_CrossReferences.HasXRefTo(rFrom);
}

bool LZ4TextDatabase::GetCrossReferenceTo(Address const& rFrom, Address::List& rToList) const
{
  std::lock_guard<std::mutex> Lock(m_CrossReferencesLock);
  return m_CrossReferences.To(rFrom, rToList);
}

bool LZ4TextDatabase::AddMultiCell(Address const& rAddress, MultiCell const& rMultiCell)
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
...
bool LZ4TextDatabase::RemoveMultiCell(Address const& rAddress)
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
...
bool LZ4TextDatabase::GetMultiCell(Address const& rAddress, MultiCell& rMultiCell) const
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
  auto itMultiCell = m_MultiCells.find(rAddress);
  if (itMultiCell == std::end(m_MultiCells))
...
bool LZ4TextDatabase::GetCellData(Address const& rAddress, CellData& rCellData)
{
  auto pMemArea = GetMemoryArea(rAddress);
  if (pMemArea == nullptr)
    return false;
...
bool LZ4TextDatabase::SetCellData(Address const& rAddress, CellData const& rCellData, Address::List& rDeletedCellAddresses, bool Force)
{
...
bool LZ4TextDatabase::DeleteCellData(Address const& rAddress)
{
  MemoryArea* pCurMemArea = nullptr;
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
  for (MemoryArea* pMemArea : m_MemoryAreas)
...
bool LZ4TextDatabase::GetComment(Address const& rAddress, std::string& rComment) const
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
...
bool LZ4TextDatabase::SetComment(Address const& rAddress, std::string const& rComment)
{
  std::lock_guard<std::mutex> Lock(m_MemoryAreaLock);
...
bool LZ4TextDatabase::_FileExists(boost::filesystem::path const& rFilePath)
{
  return boost::filesystem::exists(rFilePath);
}

bool LZ4TextDatabase::_FileRemoves(boost::filesystem::path const& rFilePath)
{
...
bool LZ4TextDatabase::_FileCanCreate(boost::filesystem::path const& rFilePath)
{
  std::ofstream File(rFilePath.string());
  return File.is_open();
}

bool LZ4TextDatabase::_MoveAddressBackward(Address const& rAddress, Address& rMovedAddress, s64 Offset) const
{
  // FIXME: Handle Offset
  if (m_MemoryAreas.empty())
    return false;
...
bool LZ4TextDatabase::_MoveAddressForward(Address const& rAddress, Address& rMovedAddress, s64 Offset) const
{
  if (m_MemoryAreas.empty())
    return false;
...
bool LZ4TextDatabase::GetValueDetail(Id ConstId, ValueDetail& rConstDtl) const
{
...
bool LZ4TextDatabase::SetValueDetail(Id ConstId, ValueDetail const& rConstDtl)
{
  m_ValuesDetail[ConstId] = rConstDtl;
  return true;
}

bool LZ4TextDatabase::GetFunctionDetail(Id FuncId, FunctionDetail& rFuncDtl) const
{
...
bool LZ4TextDatabase::SetFunctionDetail(Id FuncId, FunctionDetail const& rFuncDtl)
{
  m_FunctionsDetail[FuncId] = rFuncDtl;
  return true;
}

bool LZ4TextDatabase::GetStructureDetail(Id StructId, StructureDetail& rStructDtl) const
{
...
bool LZ4TextDatabase::SetStructureDetail(Id StructId, StructureDetail const& rStructDtl)
{
  m_StructuresDetail[StructId] = rStructDtl;
  return true;
}

bool LZ4TextDatabase::RetrieveDetailId(Address const& rAddress, u8 Index, Id& rDtlId) const
{
...
bool LZ4TextDatabase::BindDetailId(Address const& rAddress, u8 Index, Id DtlId)
{
  auto itId = m_Ids.find(rAddress);
  if (itId == std::end(m_Ids))
  {
...
bool LZ4TextDatabase::UnbindDetailId(Address const& rAddress, u8 Index)
{
  auto itId = m_Ids.find(rAddress);
  if (itId == std::end(m_Ids))
  {
